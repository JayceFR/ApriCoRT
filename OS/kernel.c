...
#define UPDATEMAP                                                              \
    ...
    for (INITMMAP; CHECKMAP; UPDATEMAP) {
      if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE) {
        ...
        printf("Available : base=0x%x%08x length=0x%x%08x\n",
               ...
               (uint32_t)(mmap->addr & 0xFFFFFFFF), (uint32_t)(mmap->len >> 32),
  ...
  process p3 = create_process(page_directory, cmain, 0);

  process p4 = create_process(page_directory, cmain, 0);

  add_process_list(head, p1);
  add_process_list(head, p2);
  add_process_list(head, p3);
  add_process_list(head, p4);
  remove_by_pid(head, 0);
  remove_by_pid(head, 1);
  remove_by_pid(head, 3);

  print_process_list(head);

  // Fail text case
  // uint32_t *ptr = (uint32_t*)0x400000; // 4MB mark (just above what you
  // mapped) *ptr = 42;
  ...
  // printf("am = 0x%x\n bm = 0x%x\n", (uint32_t)(uintptr_t)am,
  // (uint32_t)(uintptr_t)bm);
}
